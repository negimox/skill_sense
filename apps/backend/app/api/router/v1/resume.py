...
async def upload_resume(
    ...
    github_username: Optional[str] = Query(None, description="Optional GitHub username to fetch and aggregate profile data"),
    db: AsyncSession = Depends(get_db_session),
):
    """
    Accepts a PDF or DOCX file (max 2MB), converts it to HTML/Markdown, and stores it in the database.
    Optionally fetches GitHub profile data if github_username is provided.
